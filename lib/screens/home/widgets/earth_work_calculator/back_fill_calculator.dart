            ...
            children: [
              ...
              ReusableDropdown(
                ...
                onChangedCallback: (val) =>
                    controller.onMaterialTypeChanged(val),
              ),
              ReusableDropdown(
              ...
              Obx(() {
                final dataAvailable = controller.backFillCost.value != null;
                ...
                return AppButtonWidget(
                  ...
                  onPressed: () async {
                    ...
                    // ✅ Define headers & rows properly
                    final headers = ["Description", "Dimensions", "Value"];
                    final rows = [
                      [
                        ...
                        result.excavationVolume.toStringAsFixed(1).toString(),
                      ],
                      [
                        "Stone Soiling Volume (6' top layer)",
                        "--",
                        AppUtils().toRoundedDouble(result.stoneSoilingVolume).toString(),
                      ],
                      [
                        "Compacted Backfill Volume",
                        "--",
                        AppUtils().toRoundedDouble(result.compactedBackfillVolume).toString(),
                      ],
                      [
                        "Required Loose Fill Volume (95% AASHTO)",
                        "--",
                        AppUtils().toRoundedDouble(result.looseFillVolume).toString(),
                      ],
                      [
                        "Estimated Trucks Required",
                        "--",
                        "${AppUtils().toRoundedDouble(result.numberOfTrucks)} (${result.truckCapacity.toStringAsFixed(0)} ft³ each)",
                      ],
                    ];
                    ...
                    await PdfHelper.generateAndOpenPdf(
                      ...
                      inputData: {
                        'Input':
                        "L = ${controller.plotLengthController.text.isNotEmpty ? controller.plotLengthController.text : '0'} ft, "
                            "W = ${controller.plotWidthController.text.isNotEmpty ? controller.plotWidthController.text : '0'} ft, "
                            "D = ${controller.depthController.text.isNotEmpty ? controller.depthController.text : '0'} ft, "
                            "Concrete = ${controller.totalConcreteController.text.isNotEmpty ? controller.totalConcreteController.text : '0'} ft³",
                        'Soil Type': "${controller.selectedMaterialType.isNotEmpty
                            ? controller.selectedMaterialType
                            : 'N/A'}",
                        "Truck Type":
                        "${controller.selectedTruckType.isNotEmpty ? controller.selectedTruckType : 'N/A'} (${result.truckCapacity.toStringAsFixed(0)} ft³ each)",
                      },
                      headers: headers,
                      rows: rows,
              ...
              Obx(() {
                ...
                return Column(
                  children: [
                    ...
                    AppTextWidget(
                      text:
                          "Formulas: Adjusted Volume = (L)×(W)×D, Stone Soiling = (L)×(W)×0.5, Backfill = Adjusted Volume − Concrete − Stone Soiling, Loose Fill = Backfill × Compaction Factor",

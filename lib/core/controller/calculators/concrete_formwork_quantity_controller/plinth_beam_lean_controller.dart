...
class PlinthBeamLeanController extends GetxController {
  RxList<Map<String, TextEditingController>> beam = <Map<String, TextEditingController>>[].obs;

  var beamResult = Rxn<PlinthBeamLeanModel>();
  ...
  void addNewBeam() {
    beam.add({
      ...
      "height": TextEditingController(), // ✅ lowercase "height" (consistent everywhere)
  ...
  Future<void> calculate() async {
    try {
      ...
      for (int i = 0; i < beam.length; i++) {
        ...
        final height = b["height"]!.text.trim(); // ✅ lowercase
      ...
      final res = await _repo.calculatePlinthBeamLean(body: body);
      beamResult.value = PlinthBeamLeanModel.fromJson(res);
    } catch (e) {
